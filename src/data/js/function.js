$(document).ready(function () {
    ...
    $(document).on("click", "#btn-newTermin", function () {
        ...
        const newTermin = new Termin(null, date, parseInt(startTime), 0, parseInt(endTime), 0, parseInt(interval), parseInt(pizzas));
        saveTermin(newTermin);
    });
    ...
    $(document).on("click", "#btn-adminRefresh", function (){
        refresh("admin");
    });
...
async function printPizzas(termin) {
    $('#dateHead').text("").text("Abholzeiten für den " + termin.date);
    $("#dateMain").text("");
...
function makeSummary(bestellung){
    const summaryString = "<p>"+bestellung.vorn+" "+bestellung.nachn+" | "+bestellung.variante+"</p>";
...
function refresh(client){
    const clear = document.getElementById("zusammenfassung");
    clear.innerHTML = '';
    loadSite();
    switch (client){
        case "user":
